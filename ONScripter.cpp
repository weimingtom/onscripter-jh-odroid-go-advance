...
#if defined(MIYOO)
    window = SDL_CreateWindow(NULL, window_x, window_y, 480, 320, window_flag);
#else
...
#if defined(MIYOO)
    SDL_RenderSetLogicalSize(renderer, 480, 320); //Auto scale center
#else
...
#endif
#if defined(MIYOO)
    window = SDL_CreateWindow(NULL, 0, 0, 480, 320, window_flag);
#else
    window = SDL_CreateWindow(NULL, 0, 0, screen_width, screen_height, window_flag);
#endif
...
SDL_Surface *accumulation_surface2 = 0;
...
#if defined(MIYOO)    
    accumulation_surface2 = AnimationInfo::allocSurface( screen_width, screen_height, texture_format );
#endif    
...
void ONScripter::flushDirect( SDL_Rect &rect, int refresh_mode )
{
    ...
    screen_dirty_flag = false;
		...
		p.x = 0;//640 / 2;//320;
		p.y = 0;//480 / 2;//240;
		SDL_RenderCopyEx(renderer, texture, NULL, &rect_, 0, &p, SDL_FLIP_NONE);
    #else
...
#if defined(MIYOO)
if (accumulation_surface2) {
	SDL_Rect rect_2 = {0};
	rect_2.x = 0;
	rect_2.y = 0;
	rect_2.w = 640;
	rect_2.h = 480; //==screen_rect
	SDL_Rect rect_ = {0};
	rect_.x = 0;
	rect_.y = 0;
	rect_.w = 480;
	...
	SDL_Rect rect_small = {0};
if ((double) screen_rect.w / screen_rect.h > (double) 320 / 480) {
	rect_small.x = 0 + (480 - 320.0 * screen_rect.w / screen_rect.h) / 2.0;
	rect_small.y = 0;
	rect_small.w = 320.0 * screen_rect.w / screen_rect.h;
	rect_small.h = 320;
} else {
	rect_small.x = 0;
	rect_small.y = 0 + (320 - 480 * screen_rect.h / screen_rect.w) / 2.0;
	rect_small.w = 480;
	rect_small.h = 480 * screen_rect.h / screen_rect.w;
}
    SDL_BlitSurface/*SDL_BlitScaled*/(accumulation_surface, &dst_rect, accumulation_surface2, &dst_rect);
    SDL_BlitScaled(accumulation_surface2, &screen_rect/*rect_2*/, screen_surface, &rect_small/*rect_*/);
    SDL_UpdateWindowSurfaceRects(window, &rect_small/*rect_*/, 1);
} else {
...
//-------------------
//do not used, no blank scale, not good
	SDL_Rect rect_2 = {0};
	rect_2.x = 0;
	rect_2.y = 0;
	rect_2.w = 640;
	...
	rect_.w = 480;
	rect_.h = 320;
    SDL_BlitScaled(accumulation_surface, &dst_rect, accumulation_surface2, &dst_rect);
    SDL_BlitScaled(accumulation_surface2, &rect_2, screen_surface, &rect_);
//do not used, no blank scale, not good
//-------------------
